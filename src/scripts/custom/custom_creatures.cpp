...
bool GossipSelect_BarberNPC(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    if (sender == GOSSIP_SENDER_MAIN && action >= 1 && action <= 5)
    {
        ...
        for (uint8 i = 1; i <= 15; ++i)
            player->ADD_GOSSIP_ITEM(5, std::to_string(i).c_str(), category, i);
    ...
    if (sender >= 1 && sender <= 5)
    {
        ...
        // Re-open the same 1–15 menu for the same category
        for (uint8 i = 1; i <= 15; ++i)
            player->ADD_GOSSIP_ITEM(5, std::to_string(i).c_str(), sender, i);
