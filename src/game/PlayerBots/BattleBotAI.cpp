...
bool BattleBotAI::AttackStart(Unit* pVictim)
{
    ...
    if (me->IsMounted() &&
        (me->GetCombatDistance(pVictim) <= 40.0f))
...
Unit* BattleBotAI::SelectAttackTarget(Unit* pExcept) const
{
    ...
    bool ignoreCC = false;
    if (me->GetClass() == CLASS_HUNTER || me->GetClass() == CLASS_MAGE || me->GetClass() == CLASS_WARRIOR || me->GetClass() == CLASS_ROGUE)
        ignoreCC = true;
    ...
    while (pReference)
    {
        ...
        {
            if (pTarget != pExcept &&
                IsValidHostileTarget(pTarget, ignoreCC) &&
    ...
    for (const auto& pTarget : players)
    {
        ...
        if (!IsValidHostileTarget(pTarget, ignoreCC))
            continue;
            ...
            if (Unit* pMember = itr->getSource())
            {
                ...
                if (Unit* pAttacker = pMember->GetAttackerForHelper())
                {
                    if (pAttacker != pExcept &&
                        IsValidHostileTarget(pAttacker, ignoreCC) &&
...
void BattleBotAI::UpdateAI(uint32 const diff)
{
    ...
    if (!me->IsInCombat())
    {
        ...
        if (!pVictim || !IsValidHostileTarget(pVictim, ignoreCC))
        {
    ...
    else
    {
        ...
        if (!me->HasUnitState(UNIT_STAT_MELEE_ATTACKING) &&
           (m_role != ROLE_HEALER) &&
            IsValidHostileTarget(pVictim, ignoreCC) &&
            AttackStart(pVictim))
            return;
    }
            ...
            for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                if (Player* pMember = itr->getSource())
                {
                    if (me->IsValidHelpfulTarget(pMember) &&
                        ...
                        (IsPurePhysicalDPS(pMember->GetClass())))
                    {
            ...
            for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                if (Player* pMember = itr->getSource())
                {
                    if (me->IsValidHelpfulTarget(pMember) &&
                        ...
                        (IsHealerClass(pMember->GetClass()) || IsPureCasterDPS(pMember->GetClass())))
                    {
                        //here's where it'd return..
...
void BattleBotAI::UpdateInCombatAI_Shaman()
{
    ...
    if (Unit* pVictim = me->GetVictim())
    {
        ...
        if (m_spells.shaman.pFrostShock &&
            (pVictim->IsMoving() || pVictim->IsMounted() || pVictim->HasAura(AURA_SILVERWING_FLAG) || pVictim->HasAura(AURA_WARSONG_FLAG)) &&
...
void BattleBotAI::UpdateInCombatAI_Rogue()
{
    if (Unit* pVictim = me->GetVictim())
    {
            ...
            !me->CanReachWithMeleeAutoAttack(pVictim))
        {
            ...
            if (CanTryToCastSpell(pVictim, m_spells.rogue.pBlind))
            {
                if (DoCastSpell(pVictim, m_spells.rogue.pBlind) == SPELL_CAST_OK)
                {
                    return;
                }
            }
...
Player* BattleBotAI::SelectShieldTarget() const
{
    Group* pGroup = me->GetGroup();
    for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
